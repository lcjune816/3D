...
void CPLayer_RightHand::Update(_float fTimeDelta)
{
	...
	PlayerX = XMVector3Normalize(pPlayer->Get_Transform()->Get_State(STATE::RIGHT));
	PlayerY = XMVector3Normalize(pPlayer->Get_Transform()->Get_State(STATE::UP)   );
	PlayerZ = XMVector3Normalize(pPlayer->Get_Transform()->Get_State(STATE::LOOK) );
	PlayerW = pPlayer->Get_Transform()->Get_State(STATE::POS);
	...
	_vector Pos = pPlayer->Get_Transform()->Get_State(STATE::POS);
	...
	//ImGui::Begin(u8"흠흠", NULL, ImGuiWindowFlags_MenuBar); // 메뉴바임
	//ImGui::InputFloat3(u8"회전회오리",(_float*)&offsetRot);
	//ImGui::InputFloat3(u8"움직", (_float*)&offsetPos);
	//ImGui::End();
	//
	//matRot = XMMatrixRotationX(XMConvertToRadians(offsetRot.x)) * XMMatrixRotationY(XMConvertToRadians(offsetRot.y)) * XMMatrixRotationZ(XMConvertToRadians(offsetRot.z));
	...
	m_pTransform->Set_State(STATE::POS, mat.r[3]);// '//' + offsetPos);
