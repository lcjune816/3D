...
HRESULT CRollupDoor::Interaction(_float fTimeDelta, _bool bOtherTrigger)
{
	...
	case	TRIGGER_STATE::ACTION:
		_float4 fPos{};
		if (StartPos(pTransform->Get_State(STATE::POS), XMLoadFloat4(&m_vOriginPos) + XMVectorSet(0, 40.f, 0.f, 0.f), &fPos, 0.05f ))
		{
			m_eState = TRIGGER_STATE::IDLE;
	...
	case	TRIGGER_STATE::RETURN:
		...
		if (EndPos(pTransform->Get_State(STATE::POS), XMLoadFloat4(&m_vOriginPos) - XMVectorSet(0, 40.f, 0.f, 0.f), &fPos, 0.05f))
		{
			m_eState = TRIGGER_STATE::IDLE;
...
void CRollupDoor::TriggerToTrigger()
{
	...
	else
	{
		...
		{
			PLAY_SOUND(ROLLUPDOOR_SOUND, CHANNELID::SOUND_OBJECT3, 0.3f);
