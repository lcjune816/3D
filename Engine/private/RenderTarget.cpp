...
HRESULT CRenderTarget::Initialize(uint32_t iWidth, uint32_t iHeight, DXGI_FORMAT ePixelFormat, const _float4& vClearColor)
{
    ...
    TextureDesc.MipLevels = 1; //¹Ó¸Ê¾øÀ½
    TextureDesc.ArraySize = 1; 
    TextureDesc.Format = ePixelFormat;

    TextureDesc.SampleDesc.Quality = 0;
    TextureDesc.SampleDesc.Count = 1;   //MSAA ¸ÖÆ¼ ¼ÀÇÃ ¾ÈÆ¼ ¾Ù¸®¾î½Ì
...
HRESULT CRenderTarget::Ready_Debug(_float fX, _float fY, _float fSizeX, _float fSizeY)
{
    ...
    XMStoreFloat4x4(&m_WorldMatrix,
        XMMatrixScaling(fSizeX, fSizeY, 1) * XMMatrixTranslation(fX - vViewportSize.x * 0.5f, -fY + vViewportSize.y * 0.5f, 0.f));
 
    return S_OK;
}

HRESULT CRenderTarget::Debug_Render(shared_ptr<class CShader> pShader, const _char* pConstantName, shared_ptr<class CRect> pVIBuffer)
{
    if (FAILED(pShader->Bind_Matrix("g_WorldMatrix", &m_WorldMatrix)))
        return E_FAIL;

    if (FAILED(pShader->Bind_SRV(pConstantName, m_pSRV.Get())))
        return E_FAIL;
...
shared_ptr<class CRenderTarget> CRenderTarget::Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext, uint32_t iWidth, uint32_t iHeight, DXGI_FORMAT ePixelFormat, const _float4& vClearColor)
{
