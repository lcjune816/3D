...
HRESULT CGraphic_Device::Initialize(HWND hWnd, WINMODE eWinMode, uint32_t iWinSizeX, uint32_t iWinSizeY, ComPtr<ID3D11Device>& pOutDevice, ComPtr<ID3D11DeviceContext>& pOutContext)
{
	uint32_t		iFlag = 0;
	...
	m_pContext->OMSetRenderTargets(1, pRTVs, m_pDepthStencilView.Get());
...
HRESULT CGraphic_Device::Clear_BackBuffer_View(const _float4* pClearColor)
{
	...
	/* 백버퍼를 초기화한다.  */
	m_pContext->ClearRenderTargetView(m_pBackBufferRTV.Get(), reinterpret_cast<const _float*>(pClearColor));
...
unique_ptr<CGraphic_Device> CGraphic_Device::Create(HWND hWnd, WINMODE eWinMode, uint32_t iWinSizeX, uint32_t iWinSizeY, ComPtr<ID3D11Device>& pOutDevice, ComPtr<ID3D11DeviceContext>& pOutContext)
{
